...
# Use registry for dynamic classes (Always... + ForgivingTitForTat); avoids hardcoding
...
class TestStrategies(unittest.TestCase):
    ...
    def setUp(self):
        """Instantiate strategy classes from registry for testing (dynamic system/ + custom/)."""
        # Use registry to support dynamic loading/ForgivingTitForTat
        self.always_coop = STRATEGY_REGISTRY["AlwaysCooperate"]()
        self.always_defect = STRATEGY_REGISTRY["AlwaysDefect"]()
        self.tit_for_tat = STRATEGY_REGISTRY["TitForTat"]()
        self.grim_trigger = STRATEGY_REGISTRY["GrimTrigger"]()
        self.forgiver = STRATEGY_REGISTRY["ForgivingTitForTat"]()
    ...
    def test_custom_strategy_forgiving_tit_for_tat(self):
        ...
        # Start C
        self.assertEqual(self.forgiver.decide([]), "C")
        # Mirror
        self.assertEqual(self.forgiver.decide(["D"]), "D")
        # Forgive after two D's
        self.assertEqual(self.forgiver.decide(["D", "D"]), "C")
        self.assertEqual(self.forgiver.decide(["C", "D", "D"]), "C")
